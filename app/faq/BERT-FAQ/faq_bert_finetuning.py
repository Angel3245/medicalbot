...
class FAQ_BERT_Finetuning(object):
    ...
    def __init__(self, loss_type="triplet", query_type='faq', neg_type='simple', version="1.1", epochs=4, batch_size=32, 
                 pre_trained_name='distilbert-base-uncased', evaluation_steps=1000, test_size=0.20, num_labels=1, learning_rate=5e-5, adam_lrs=5e-5,
                 adam_epsilon=1e-8):
    ...
    def train(self, bert_model, train_samples, val_samples, output_path):
        ...
        if self.loss_type == "triplet":
            ...
            model = bert_model.fit(
                ...
                warmup_steps=warmup_steps, output_path=output_path, optimizer_params={'lr': self.learning_rate, 'eps': self.adam_epsilon}
            )
        elif self.loss_type == "softmax":
            # generate train dataloader
            train_dataloader = DataLoader(train_samples, shuffle=True, batch_size=self.batch_size, num_workers=8)
